...
const VALID_USER_ROLES = ['PMO', 'Executive', 'CSM', 'PM', 'TL', 'HR', 'Assignee'];
...
function assertManagedRole(role) {
  ...
  if (!VALID_USER_ROLES.includes(normalized)) {
    throw new Error(`Role must be one of: ${VALID_USER_ROLES.join(', ')}`);
  }
  return normalized;
}
...
async function syncUserProfile(sql, { email, name, role, active = true, azumoId = null }) {
  ...
  await sql`
    INSERT INTO ${qualified(sql, 'users')} AS users (email, azumo_id, first_name, last_name, role, is_active, updated_at)
    VALUES (
      ${normalizedEmail},
      ${azumoId ?? null},
      ${firstName || null},
...
async function getUserByEmail(email) {
  ...
  const rows = await sql`
    SELECT
      ...
      p.password_hash,
      p.last_login_at
    FROM ${qualified(sql, 'pmo_users')} p
    LEFT JOIN ${qualified(sql, 'users')} u ON u.email = p.email
...
async function getManagedUserByEmail(email) {
  const sql = await sqlRequired();
  const rows = await sql`
    SELECT
      COALESCE(u.id::text, p.id) AS id,
      ...
      COALESCE(u.is_active, p.active) AS active,
      p.last_login_at
    FROM ${qualified(sql, 'users')} u
    FULL OUTER JOIN ${qualified(sql, 'pmo_users')} p ON p.email = u.email
    WHERE COALESCE(u.email, p.email) = ${normalizeEmail(email)}
    LIMIT 1
  `;
...
async function ensureAuthRecordForUser(sql, user) {
  ...
  const rows = await sql`
    INSERT INTO ${qualified(sql, 'pmo_users')} (id, email, name, role, password_hash, active)
...
async function issueSession(sql, authUserId) {
  ...
  await sql`DELETE FROM ${qualified(sql, 'pmo_sessions')} WHERE expires_at < now()`;
  await sql`
    INSERT INTO ${qualified(sql, 'pmo_sessions')} (id, user_id, token_hash, impersonated_user, expires_at)
    VALUES (${sessionId}, ${authUserId}, ${hash}, ${null}, ${expiresAt.toISOString()})
  `;
  return token;
}
...
async function createManagedUser({ email, first_name, last_name, role = 'Executive', is_active = true, azumo_id = null }) {
  const normalizedEmail = normalizeEmail(email);
  if (!normalizedEmail) throw new Error('Email is required.');
  ...
  const sql = await sqlRequired();
  const existing = await getAuthorizedUserByEmail(normalizedEmail);
  ...
  const rows = await sql`
    ...
    RETURNING id::text AS id, email, first_name, last_name, azumo_id, role, is_active
  `;
  await ensureAuthRecordForUser(sql, rows[0]);
  return safeUser({
    ...rows[0],
...
async function updateManagedUser({ email, role, is_active }) {
  const normalizedEmail = normalizeEmail(email);
  if (!normalizedEmail) throw new Error('Email is required.');
  const shouldUpdateRole = role !== undefined;
  const shouldUpdateActive = is_active !== undefined;
  if (!shouldUpdateRole && !shouldUpdateActive) throw new Error('Nothing to update.');

  const sql = await sqlRequired();
  const current = await getManagedUserByEmail(normalizedEmail);
  if (!current) throw new Error('User not found.');

  const nextRole = shouldUpdateRole ? assertManagedRole(role) : canonicalRole(current.role);
  const nextActive = shouldUpdateActive ? Boolean(is_active) : Boolean(current.active !== false);
  const split = splitDisplayName(current.name || '', normalizedEmail);
  const firstName = String(current.first_name || split.firstName || '').trim() || null;
  const lastName = String(current.last_name || split.lastName || '').trim() || null;

  await sql`
    INSERT INTO ${qualified(sql, 'users')} AS users (email, azumo_id, first_name, last_name, role, is_active, updated_at)
    VALUES (
      ${normalizedEmail},
      ${current.azumo_id ?? null},
      ${firstName},
      ${lastName},
      ${nextRole},
      ${nextActive},
  ...
  const authRecord = await getAuthorizedUserByEmail(normalizedEmail);
  if (authRecord) await ensureAuthRecordForUser(sql, authRecord);
  return getManagedUserByEmail(normalizedEmail);
}
...
async function login(email, password) {
  ...
  await sql`UPDATE ${qualified(sql, 'pmo_users')} SET last_login_at = now(), updated_at = now() WHERE id = ${authUserId}`;
...
async function fetchSessionUserByEmail(sql, email) {
  const rows = await sql`
    SELECT
      COALESCE(u.id::text, p.id) AS id,
      COALESCE(u.email, p.email) AS email,
...
async function getSessionContext(req) {
  if (process.env.VERCEL_ENV === 'preview' && process.env.PMO_DEV_BYPASS === 'true') {
    // Role defaults to PM so most preview links behave like a normal team member.
    // Set PMO_DEV_ROLE (Preview scope, e.g. "PMO") in Vercel to test PMO/admin-only
    // UI — such as the Processes "Nuevo/Editar proceso" form — on a preview deploy,
    // since real Google login is bypassed here and can't otherwise reach that role.
    const fakeUser = {
      id: 'preview-dev-user',
      email: 'micaela.goicoechea@azumo.co',
      name: 'Preview Dev User',
      role: process.env.PMO_DEV_ROLE || 'PM',
      active: true
    };
    return {
      user: fakeUser,
  ...
  const rows = await sql`
    SELECT
      ...
      COALESCE(u.is_active, p.active) AS active,
      p.last_login_at
    FROM ${qualified(sql, 'pmo_sessions')} s
    JOIN ${qualified(sql, 'pmo_users')} p ON p.id = s.user_id
    LEFT JOIN ${qualified(sql, 'users')} u ON u.email = p.email
  ...
  const impersonation = parseJsonObject(row.impersonated_user);
  const impersonatedEmail = normalizeEmail(
    impersonation?.email
    || impersonation?.user?.email
    || impersonation?.impersonated_user?.email
  );
  let impersonatedUser = null;
  if (impersonatedEmail) {
    const candidate = await fetchSessionUserByEmail(sql, impersonatedEmail);
    if (candidate && candidate.active !== false && !isPmoRole(candidate.role)) {
      impersonatedUser = candidate;
    } else {
      await sql`UPDATE ${qualified(sql, 'pmo_sessions')} SET impersonated_user = NULL WHERE id = ${row.session_id}`;
    }
  }

  return {
    user: impersonatedUser || realUser,
    realUser,
    impersonatedUser,
...
async function setSessionImpersonatedUser(req, email) {
  const context = await getSessionContext(req);
  if (!context.realUser || context.realUser.active === false || !isPmoRole(context.realUser.role)) {
    throw new Error('Administrator access required.');
  }
  if (!context.sessionId || !context.sql) {
    throw new Error('Active session required.');
  }

  const target = await fetchSessionUserByEmail(context.sql, email);
  if (!target || target.active === false) {
    throw new Error('User not found or inactive.');
  }
  if (isPmoRole(target.role)) {
    throw new Error('PMO users cannot be impersonated.');
  }

  await context.sql`
    UPDATE ${qualified(context.sql, 'pmo_sessions')}
    SET impersonated_user = ${JSON.stringify({ email: target.email })}::jsonb
    WHERE id = ${context.sessionId}
  `;
  return target;
}

async function clearSessionImpersonatedUser(req) {
  const context = await getSessionContext(req);
  if (!context.sessionId || !context.sql) return null;
  await context.sql`UPDATE ${qualified(context.sql, 'pmo_sessions')} SET impersonated_user = NULL WHERE id = ${context.sessionId}`;
  return context.realUser || null;
}
...
async function logout(req) {
  ...
  if (!sql) return;
  await ensureSchema(sql);
  await sql`DELETE FROM ${qualified(sql, 'pmo_sessions')} WHERE token_hash = ${tokenHash(token)}`;
}
...
async function changePassword(req, currentPassword, newPassword) {
  ...
  await sql`
    UPDATE ${qualified(sql, 'pmo_users')}
  ...
  await sql`DELETE FROM ${qualified(sql, 'pmo_sessions')} WHERE user_id = ${user.auth_id || user.id} AND token_hash <> ${hash}`;
