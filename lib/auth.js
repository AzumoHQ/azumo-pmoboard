...
async function issueSession(sql, authUserId) {
  ...
  await sql`
    INSERT INTO pmo_sessions (id, user_id, token_hash, impersonated_user, expires_at)
...
async function getSessionContext(req) {
  ...
  if (impersonatedEmail) {
    ...
    if (candidate && candidate.active !== false && !isPmoRole(candidate.role)) {
      ...
      await sql`UPDATE pmo_sessions SET impersonated_user = NULL WHERE id = ${row.session_id}`;
    }
  }
...
async function clearSessionImpersonatedUser(req) {
  ...
  await context.sql`UPDATE pmo_sessions SET impersonated_user = NULL WHERE id = ${context.sessionId}`;
