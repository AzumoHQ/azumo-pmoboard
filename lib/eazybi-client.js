...
const EAZYBI_BENCH_BY_MONTH_ACCOUNT_ID = process.env.EAZYBI_BENCH_BY_MONTH_ACCOUNT_ID || '232624';
const EAZYBI_UTILIZATION_BILLING_ACCOUNT_ID = process.env.EAZYBI_UTILIZATION_BILLING_ACCOUNT_ID || '232624';
const EAZYBI_UTILIZATION_BILLING_REPORT_ID = process.env.EAZYBI_UTILIZATION_BILLING_REPORT_ID || '5244970';
...
function isBenchAvailabilityMeasure(measure) {
  const text = String(measure || '').trim();
  return /availability/i.test(text) || /available\s+capacity/i.test(text) || /avaialable\s+capacity/i.test(text);
}

function isBenchUtilizationMeasure(measure) {
  return /assignment\s+utilization/i.test(String(measure || '').trim()) || /^utilization$/i.test(String(measure || '').trim());
}
...
function parseBenchByMonthReport(report) {
  ...
  const months = [...new Set(columns
    ...
    .filter((column) => (isBenchAvailabilityMeasure(column.measure) || isBenchUtilizationMeasure(column.measure)) && column.month)
  ...
  for (const row of rows) {
    ...
    for (const column of columns) {
      ...
      if (!month) continue;
      if (isBenchAvailabilityMeasure(measure)) parsed.availability[month] = percentValue(row[column]);
      if (isBenchUtilizationMeasure(measure)) parsed.utilization[month] = percentValue(row[column]);
    }
...
async function fetchBenchByMonthReport(
  reportId = EAZYBI_BENCH_BY_MONTH_REPORT_ID,
  accountId = EAZYBI_BENCH_BY_MONTH_ACCOUNT_ID || EAZYBI_DEFAULT_ACCOUNT_ID,
  embedToken = EAZYBI_BENCH_BY_MONTH_TOKEN
) {
  if (!reportId) return null;
  let report;
  if (embedToken) {
    try {
      report = await exportEmbeddedReport(reportId, accountId, embedToken);
    } catch (error) {
      report = await exportReport(reportId, accountId);
    }
  } else {
    report = await exportReport(reportId, accountId);
  }
  return parseBenchByMonthReport(report);
}
