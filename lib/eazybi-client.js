...
const EAZYBI_BENCH_BY_MONTH_REPORT_ID = process.env.EAZYBI_BENCH_BY_MONTH_REPORT_ID;
...
const EAZYBI_BENCH_BY_MONTH_ACCOUNT_ID = process.env.EAZYBI_BENCH_BY_MONTH_ACCOUNT_ID || process.env.EAZYBI_ACCOUNT_ID;
...
const EAZYBI_INTERNAL_PROJECTS_ACCOUNT_ID = process.env.EAZYBI_INTERNAL_PROJECTS_ACCOUNT_ID;
const EAZYBI_INTERNAL_PROJECTS_REPORT_ID = process.env.EAZYBI_INTERNAL_PROJECTS_REPORT_ID;
const EAZYBI_INTERNAL_PROJECTS_TOKEN = process.env.EAZYBI_INTERNAL_PROJECTS_TOKEN;
const EAZYBI_NEW_SEARCHES_ACCOUNT_ID = process.env.EAZYBI_NEW_SEARCHES_ACCOUNT_ID;
const EAZYBI_NEW_SEARCHES_REPORT_ID = process.env.EAZYBI_NEW_SEARCHES_REPORT_ID;
...
function internalProjectPercent(rawValues, formattedValues, columns, rowIndex, match) {
  const index = columns.findIndex(match);
...
async function fetchInternalProjectsReport(
  ...
  if (!reportId || !accountId || !embedToken) return null;
  return parseInternalProjectsReport(await exportEmbeddedReport(reportId, accountId, embedToken));
}
...
function parseNewSearchesTriageReport(report) {
  ...
  const rawValues = queryResults.values || [];
  const formattedValues = queryResults.formatted_values || [];
  const columnIndex = (match) => columns.findIndex(match);
  const idx = {
...
async function fetchNewSearchesTriageReport(
  ...
  if (!reportId || !accountId || !embedToken) return null;
  return parseNewSearchesTriageReport(await exportEmbeddedReport(reportId, accountId, embedToken));
}
...
function clientReportConfig() {
  return {
    ...
    new_searches: {
      source_link: buildReportUrl(EAZYBI_NEW_SEARCHES_ACCOUNT_ID, EAZYBI_NEW_SEARCHES_REPORT_ID),
      embed_path: '/api/eazybi-embed?report=new_searches'
    }
  };
}
