...
function fieldEmail(value) {
  ...
  return String(value.emailAddress || '').trim().toLowerCase();
}
  ...
  const rawAssignments = rawIssues
    .map((issue) => {
      ...
      return {
        ...
        project_manager: fieldValue(parentFields[CF_PROJECT_MANAGER]) || fieldValue(fields[CF_PROJECT_MANAGER]),
        epic_key: parent.key || '',
...
// Replica exacto el criterio de billingBenchRows() en index.html: mismo
// chequeo de status, mismo orden de campos para resolver el nombre, y
// misma clave de agrupacion. Se mantiene asi -- no delegar a
// personMatchKey() -- para garantizar que metrics.bench y el hover de
// "Active Bench people" siempre coincidan, sin duplicar la logica con
// una definicion ligeramente distinta.
function countActiveBenchPeople(rows = []) {
  const seen = new Set();
  rows.forEach((row = {}) => {
...
function coverageRowFromIssue(issue) {
  ...
  const pmEmail = fieldEmail(fields[CF_COVERAGE_PM]) || fieldEmail(fields[CF_PROJECT_MANAGER]);
  const csmEmail = fieldEmail(fields[CF_COVERAGE_CSM]);
  ...
  return {
    ...
    csm_assigned: csm || '',
    tl_assigned: tl || '',
    pm_assigned_email: pmEmail || '',
    csm_assigned_email: csmEmail || '',
...
function applyEnrichment(assignment, enrichment) {
  ...
  if (!enriched.project_manager && row.project_manager) enriched.project_manager = row.project_manager;
  if (row.freelance) enriched.freelance = row.freelance;
  // Only backfill from the previous snapshot when the freshly-parsed Jira
  // value is missing. These used to overwrite unconditionally, which meant
  // a just-updated Jira field (e.g. Billing going from 0 to 100) got
  // clobbered back to the old value on the very next refresh, because the
  // previous snapshot still had the stale number on file.
  if ((enriched.epic_billing === undefined || enriched.epic_billing === '') && row.epic_billing !== undefined && row.epic_billing !== '') {
    enriched.epic_billing = row.epic_billing;
  }
  ...
  if ((enriched.availability_pct === undefined || enriched.availability_pct === '') && row.availability_pct !== undefined && row.availability_pct !== '') {
    enriched.availability_pct = row.availability_pct;
  }
  ...
  if ((enriched.billing_pct === undefined || enriched.billing_pct === '') && row.billing_pct !== undefined && row.billing_pct !== '') {
    enriched.billing_pct = row.billing_pct;
  }
  if ((enriched.bench_pct === undefined || enriched.bench_pct === '') && row.bench_pct !== undefined && row.bench_pct !== '') {
    enriched.bench_pct = row.bench_pct;
  }
  return normalizeInternalCapacity({
    ...enriched,
...
function buildSnapshot(parsed, overrides = {}) {
  ...
  const snapshot = {
    ...
    metrics: {
      ...
      bench: countActiveBenchPeople(parsed.bench || []) || Number(overrides.bench || 0),
    ...
    harvest_metrics: parsed.harvest_metrics || {},
    assignment_rows: [
      ...(parsed.active || []),
      ...(parsed.bench || []),
      ...(parsed.pending || [])
...
function buildActivityLog(newAssignments, prevSnapshot, psaProjects, daysBack, newHarvestMetrics) {
  ...
  var prevRows = (prevSnapshot && prevSnapshot.assignment_rows) || [];
  var prevMap = new Map();
  prevRows.forEach(function(r) { prevMap.set((r.key||'')+'|'+(r.client||''), r); });
  var newKeys = new Set();
  (newAssignments||[]).forEach(function(r) {
    var k = (r.key||'')+'|'+(r.client||'');
    newKeys.add(k);
    var prev = prevMap.get(k);
    var ts = r.updated ? new Date(r.updated) : null;
    if (ts && ts < cutoff) return;
    var iso = ts ? ts.toISOString() : now;
    var base = {ts:iso, assignee:r.assignee||'', client:r.client||'',
                project_manager:r.project_manager||'', key:r.key||''};
    if (!prev) {
      events.push(Object.assign({},base,{type:'new_assignment',
        label:(r.assignee||'')+' assigned to '+(r.client||'')+
              (r.assignment_pct?' ('+r.assignment_pct+'%)':'')}));
    } else {
      // Pending → In Progress (assignment activated)
      if (prev.status==='Assigned'&&r.status==='In Progress')
        events.push(Object.assign({},base,{type:'status_change',
          label:(r.assignee||'')+' started at '+(r.client||'')+
                (r.assignment_pct?' ('+r.assignment_pct+'%)':'')}));
      if (r.assignment_pct!==prev.assignment_pct&&r.assignment_pct!=null&&prev.assignment_pct!=null)
        events.push(Object.assign({},base,{type:'pct_change',
          label:(r.assignee||'')+' @ '+(r.client||'')+': allocation '+
                prev.assignment_pct+'% → '+r.assignment_pct+'%'}));
      if (r.due&&prev.due&&r.due!==prev.due)
        events.push(Object.assign({},base,{type:'due_change',
          label:(r.assignee||'')+' @ '+(r.client||'')+': due '+prev.due+' → '+r.due}));
      var oldPm = (prev.project_manager||'').trim();
      var newPm = (r.project_manager||'').trim();
      if (oldPm && newPm && oldPm !== newPm)
        events.push(Object.assign({},base,{type:'pm_change',
          label:(r.assignee||'')+' @ '+(r.client||'')+': PM '+oldPm+' → '+newPm}));
    }
  });
  // Detect assignments that disappeared since last snapshot → moved to Done/Cancelled
  prevMap.forEach(function(prev, k) {
    if (!newKeys.has(k))
      events.push({ts:now, type:'assignment_done', assignee:prev.assignee||'',
        client:prev.client||'', project_manager:prev.project_manager||'', key:prev.key||'',
        label:(prev.assignee||'')+' finished at '+(prev.client||'')});
  });
  (psaProjects||[]).forEach(function(proj) {
    (proj.reports||[]).forEach(function(rep) {
      var updStr = rep.fields&&rep.fields.updated;
      if (!updStr) return;
      var ts = new Date(updStr);
      if (ts<cutoff) return;
      var summary = (rep.fields&&rep.fields.summary)||rep.key||'';
      events.push({ts:ts.toISOString(), type:'psa_report',
        project:proj.name||proj.key||'', project_manager:proj.project_manager||'',
        client:proj.client||proj.name||'',
        label:'PSA report: '+summary+' ('+(proj.name||proj.key||'')+')',
        key:rep.key||'', assignee:''});
    });
  });
  // Harvest changes — compares consecutive snapshots regardless of whether the week changed
  var prevHarvest = (prevSnapshot && prevSnapshot.harvest_metrics) || null;
  var sameHarvestWeek = prevHarvest && newHarvestMetrics &&
    prevHarvest.range && newHarvestMetrics.range &&
    prevHarvest.range.from === newHarvestMetrics.range.from;
  if (newHarvestMetrics && newHarvestMetrics.by_person && prevHarvest && prevHarvest.by_person) {
    var harvestTs = (newHarvestMetrics.range && newHarvestMetrics.range.to)
      ? new Date(newHarvestMetrics.range.to + 'T23:59:00Z').toISOString()
      : now;
    var prevByName = new Map();
    (prevHarvest.by_person || []).forEach(function(p) {
      if (p.user_name) prevByName.set(p.user_name.trim().toLowerCase(), p);
    });
    var BENCH_THRESHOLD    = 10;
    var INTERNAL_THRESHOLD = 10;
    var HOURS_CHANGE_MIN   = 2;   // ≥2h change in committed hours is noteworthy within same week
    (newHarvestMetrics.by_person || []).forEach(function(cur) {
      var key = (cur.user_name || '').trim().toLowerCase();
      if (!key) return;
      var prev = prevByName.get(key);
      if (!prev) return;
      var curCommitted  = Number(cur.committed_harvest_hours || 0);
      var prevCommitted = Number(prev.committed_harvest_hours || 0);
      var curBench      = Number(cur.bench_hours || 0);
      var curInternal   = Number(cur.internal_projects_hours || 0);
      var curClient     = Number(cur.client_logged_hours || 0);
      var prevBench     = Number(prev.bench_hours || 0);
      var prevInternal  = Number(prev.internal_projects_hours || 0);
      var prevClient    = Number(prev.client_logged_hours || 0);
      var name = cur.user_name || '';

      if (sameHarvestWeek) {
        // Within the same tracked week: only flag significant hour edits (timesheet corrections)
        var delta = curCommitted - prevCommitted;
        if (Math.abs(delta) >= HOURS_CHANGE_MIN) {
          events.push({ts: now, type: 'harvest_hours_edit', assignee: name, client: '',
            label: name + ': hours ' + (delta > 0 ? '+' : '') + delta.toFixed(2) + 'h'
              + ' (' + prevCommitted.toFixed(1) + 'h → ' + curCommitted.toFixed(1) + 'h)'
              + ' — week ' + (newHarvestMetrics.range ? newHarvestMetrics.range.from : '')});
        }
      } else {
        // Week changed: detect status transitions
        if (curBench >= BENCH_THRESHOLD && prevBench < BENCH_THRESHOLD && prevClient >= BENCH_THRESHOLD)
          events.push({ts: harvestTs, type: 'harvest_to_bench', assignee: name, client: '',
            label: name + ' moved to bench (' + curBench.toFixed(1) + 'h bench this week)'});
        if (prevBench >= BENCH_THRESHOLD && curBench < BENCH_THRESHOLD && curClient >= BENCH_THRESHOLD)
          events.push({ts: harvestTs, type: 'harvest_from_bench', assignee: name, client: '',
            label: name + ' back to client work (' + curClient.toFixed(1) + 'h billable)'});
        if (curInternal >= INTERNAL_THRESHOLD && prevInternal < INTERNAL_THRESHOLD)
          events.push({ts: harvestTs, type: 'harvest_to_internal', assignee: name, client: '',
            label: name + ' on internal projects (' + curInternal.toFixed(1) + 'h this week)'});
        if (prevInternal >= INTERNAL_THRESHOLD && curInternal < INTERNAL_THRESHOLD && curClient >= BENCH_THRESHOLD)
          events.push({ts: harvestTs, type: 'harvest_from_internal', assignee: name, client: '',
            label: name + ' back to client work from internal (' + curClient.toFixed(1) + 'h billable)'});
      }
    });
  }
