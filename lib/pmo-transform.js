...
function buildActivityLog(newAssignments, prevSnapshot, psaProjects, daysBack) {
  ...
  (newAssignments||[]).forEach(function(r) {
    ...
    var iso = ts ? ts.toISOString() : new Date().toISOString();
    ...
    if (!prev) {
      ...
      if (r.status==='Done'&&prev.status!=='Done')
        events.push(Object.assign({},base,{type:'assignment_done',
          label:(r.assignee||'')+' finished at '+(r.client||'')}));
