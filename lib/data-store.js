...
async function ensureSchema(sql) {
  // Create the target schema first so the unqualified CREATE TABLEs below land in it
  // (search_path is set to `${DB_SCHEMA},public` on the connection).
  await sql`CREATE SCHEMA IF NOT EXISTS pmo`;
  ...
  `;
  await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS azumo_id INTEGER`;
  ...
  `;
  await sql`ALTER TABLE pmo_sessions ADD COLUMN IF NOT EXISTS impersonated_user jsonb`;
