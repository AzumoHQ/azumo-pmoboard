...
async function ensureSchema(sql) {
  ...
  // Admin: company roles, people directory and audit log (see api/admin.js, lib/admin-store.js).
  // company_roles  = catalog of company positions ("CSM", "Head of HR"...). Seeded from the Jira
  //                  AA Epic field "Position - Assignee"; coverage_slot marks which roles are valid
  //                  options for an account's PM / CSM / TL "responsable".
  // company_people = one row per AA Epic (one Epic = one employee). role_id is a manual override;
  //                  when empty the effective role is the one whose name matches jira_position.
  // admin_audit_log = every edit made from /admin, including whether it was written to Jira.
  await sql`
    CREATE TABLE IF NOT EXISTS ${q('company_roles')} (
      id serial PRIMARY KEY,
  ...
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS ${q('company_people')} (
      jira_epic_key text PRIMARY KEY,
      name text NOT NULL,
      email text,
      jira_account_id text,
      jira_position text,
      position_field_kind text,
      role_id integer REFERENCES ${q('company_roles')}(id) ON DELETE SET NULL,
      freelance text,
      start_date text,
      status text,
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS idx_company_people_email ON ${q('company_people')}(lower(email))`;
  await sql`
    CREATE TABLE IF NOT EXISTS ${q('admin_audit_log')} (
      id serial PRIMARY KEY,
