...
// "Bench" y "Azumo" no son clientes reales: son las convenciones que ya usa
// el resto del board (ver harvestHoursActiveClientNames() en index.html)
// para marcar tiempo ocioso y proyectos internos. Todo lo demas es trabajo
// para un cliente real, separado por el flag billable de la propia entrada
// de Harvest (autoritativo, no el is_billable a nivel de proyecto).
function categorizeHarvestEntry(entry = {}) {
  const clientName = clean(entry.client?.name).toLowerCase();
  if (clientName === 'bench') return { bucket: 'bench', subcategory: '' };
  if (clientName === 'azumo') {
    return { bucket: 'internal_projects', subcategory: clean(entry.project?.name) || 'Sin proyecto' };
  }
  if (entry.billable === true) return { bucket: 'billable', subcategory: '' };
  return { bucket: 'other_non_billable', subcategory: '' };
}

async function fetchWeeklyHarvestMetrics({ from, to }) {
  const entries = await fetchPaged('/time_entries', 'time_entries', { from, to });

  const totals = { billable: 0, bench: 0, internal_projects: 0, other_non_billable: 0 };
  const internalByProject = {};
  const byPersonMap = new Map();

  for (const entry of entries) {
    const { bucket, subcategory } = categorizeHarvestEntry(entry);
    const hours = Number(entry.hours || 0);
    const userId = entry.user?.id || '';
    const userName = clean(entry.user?.name) || 'Sin nombre';

    totals[bucket] += hours;
    if (bucket === 'internal_projects') {
      const key = subcategory || 'Sin proyecto';
      internalByProject[key] = (internalByProject[key] || 0) + hours;
    }
  ...
  const byPerson = Array.from(byPersonMap.values())
    .map((row) => ({
      ...row,
      billable: round1(row.billable),
      bench: round1(row.bench),
      internal_projects: round1(row.internal_projects),
      other_non_billable: round1(row.other_non_billable),
      total: round1(row.billable + row.bench + row.internal_projects + row.other_non_billable)
    }))
    .sort((a, b) => a.user_name.localeCompare(b.user_name));

  const totalHours = totals.billable + totals.bench + totals.internal_projects + totals.other_non_billable;
  ...
  return {
    ...
    billable_hours: round1(totals.billable),
    bench_hours: round1(totals.bench),
    internal_projects_hours: round1(totals.internal_projects),
    other_non_billable_hours: round1(totals.other_non_billable),
    internal_projects_by_project: Object.fromEntries(
      Object.entries(internalByProject).map(([key, value]) => [key, round1(value)])
    ),
    by_person: byPerson
  };
}
