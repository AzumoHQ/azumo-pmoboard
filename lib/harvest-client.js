...
// Wagner-Fischer Levenshtein — no external dependency, ~20 lines.
...
/**
 * Produces a `health_checks` array of structured diagnostic objects from the
 * data already computed by fetchWeeklyHarvestMetrics().  Purely read-only —
 * does not alter totals or categorisation.
 *
 * {severity: 'error'|'warning', type, message, person?}
 *
 * Checks:
 *   1. identity_imbalance   — buckets don't add up to committed (error)
 *   2. no_hours_logged      — roster member logged nothing this week (warning)
 *   3. possible_identity_mismatch — excluded name is close to a roster name (warning)
 *   4. billable_timeoff_overlap  — mirrors existing string warnings (warning)
 */
function computeDataHealthChecks({ metrics, roster, byPerson, usersById, excluded, warnings }) {
  const checks = [];

  // 1. Identity imbalance
  const identity = round2(
    metrics.client_logged_hours
    + metrics.internal_projects_logged_hours
    + metrics.internal_bench_hours
    + metrics.total_off_hours
  );
  const delta = round2(Math.abs(metrics.committed_harvest_hours - identity));
  if (delta > 0.05) {
    checks.push({
      severity: 'error',
      type: 'identity_imbalance',
      message: `Totals don\'t balance: committed ${metrics.committed_harvest_hours} h ≠ `
        + `client_logged + internal_projects + bench + time_off = ${identity} h `
        + `(delta ${delta} h).`
    });
  }

  // 2. Roster members with zero hours logged this week
  if (roster) {
    const loggedEmails = new Set(byPerson.map((p) => p.email).filter(Boolean));
    ...
    for (const email of roster) {
      if (!loggedEmails.has(email)) {
        const displayName = emailToName.get(email) || email;
        checks.push({
          severity: 'warning',
          type: 'no_hours_logged',
          message: `${displayName} is on the billable roster but logged no hours this week.`,
          person: displayName
        });
      }
    }
  ...
  // 3. Possible identity mismatches between excluded people and roster members
  if (roster && excluded.length > 0) {
    const rosterPeople = [];
    for (const u of usersById.values()) {
      if (u.email && roster.has(u.email.toLowerCase())) {
        rosterPeople.push({ name: u.name || u.email, norm: normalizeName(u.name || u.email) });
      }
    }
    for (const excl of excluded) {
      const exclNorm = normalizeName(excl.user_name);
      for (const rp of rosterPeople) {
        if (exclNorm === rp.norm) continue; // exact match — same person, already handled
        const dist = levenshtein(exclNorm, rp.norm);
        const maxLen = Math.max(exclNorm.length, rp.norm.length);
        if (maxLen > 0 && dist / maxLen < 0.2) {
          checks.push({
            severity: 'warning',
            type: 'possible_identity_mismatch',
            message: `\'${excl.user_name}\' (Harvest, excluded) may be the same person as \'${rp.name}\' (roster) — check for a data entry mismatch.`,
            person: excl.user_name
          });
        }
      }
    }
  ...
  // 4. Mirror billable_timeoff_overlap warnings from the string array
  for (const w of warnings) {
    if (w.startsWith('Billable entry looks like time off:')) {
      checks.push({ severity: 'warning', type: 'billable_timeoff_overlap', message: w });
    }
  }
...
async function fetchWeeklyHarvestMetrics({ from, to }, options = {}) {
  ...
  for (const entry of entries) {
    ...
    if (!byPersonMap.has(personKey)) {
      byPersonMap.set(personKey, {
        ...
        bench: 0,
        time_off: 0,
        internal_projects_detail: {},
        bench_detail: {}
      });
    }
    byPersonMap.get(personKey)[bucket] += hours;
    if (bucket === 'internal_projects') {
      const proj = subcategory || 'Unspecified';
      const pd = byPersonMap.get(personKey).internal_projects_detail;
      pd[proj] = (pd[proj] || 0) + hours;
    } else if (bucket === 'bench') {
      const proj = subcategory || 'Unspecified';
      const bd = byPersonMap.get(personKey).bench_detail;
      bd[proj] = (bd[proj] || 0) + hours;
    }
  }
  ...
  const byPerson = Array.from(byPersonMap.values())
    .map((row) => ({
      ...
      internal_projects_detail: sortedHourMap(row.internal_projects_detail),
      bench_detail: sortedHourMap(row.bench_detail)
    }))
  ...
  metrics.health_checks = computeDataHealthChecks({
    metrics,
    roster,
    byPerson,
    usersById,
